...
myUnknown.it.is.possible.to.access.any.deep.property //✔️ Fails as it should - needs typeguard 
...
//* Practical use of top types: unknown 
function doSomethingRisky() {
  if (Math.random() > 0.5) return 'ok'
  else if (Math.random() > 0.5) throw new Error('Bad luck!')
  else throw 'Really bad luck'
}
// Note: e: unknown -> error checking 
// alternative: in tsconfig.ts add "useUnknownInCatchVariables": true  
try {
  doSomethingRisky()
} catch (e: unknown) {
...
// let nullableString: string | null = 'not null'
// nullableString = null
